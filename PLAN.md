...
| Airports / distance | **OpenFlights** airport table (public domain) + haversine | Generated to `src/core/data/airports.json` (6,071 airports w/ resolved ISO3) via `npm run gen:airports`; lazy-loaded like the map data. No network at runtime. |
...
| **3. Miles** | ✅ Done — `src/core/data/airports.json` generated from OpenFlights (6,071 airports, `npm run gen:airports`); Add-flight screen with IATA/city autocomplete → haversine → cached `distanceMiles`; flights soft-mark their countries as "layover" on the map (only where no real visit exists); legs list has delete + real "hours in the air" estimate. | done |
...
`src/core/data/countries.ts` is **generated** from the `world-countries` dev dependency —
run `npm run gen:countries` to refresh; don't hand-edit.
